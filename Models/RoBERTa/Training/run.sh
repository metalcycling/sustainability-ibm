...
do
    ...
    elif [[ ${action} == "user-file" ]]; then
        helm uninstall roberta-user-file
        helm upgrade --install --wait --set namespace=${namespace} --set jobName="roberta-user-file" --set volumes[0].claimName=${claim_name} -f roberta.yaml roberta-user-file /home/metalcycling/Programs/Foundation_Model_Stack/Fork/tools/scripts/appwrapper-pytorchjob/chart
