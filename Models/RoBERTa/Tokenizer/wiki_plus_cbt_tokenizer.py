"""
Wiki + CBT Tokenizer Training:

We will begin by training a tokenizer on our desired dataset. A combined Wikipedia and Children's Book Test (CBT) dataset is used in this example. HuggingFace provides the tokenizer, and in this case the dataset as well. We will take HuggingFace's base RoBERTa tokenizer and retrain it on CBT.
"""
...
wiki_shards = os.listdir("./wiki_shards")
cbt_shards = os.listdir("./cbt_shards")
min_paragraph_length = 10

def batch_iterator():
    for shard_id in range(num_shards):
        with open("./wiki_shards/wiki_shard_%d.pkl" % (shard_id), "rb") as fileptr:
            wiki_dataset = pickle.load(fileptr)

        with open("./cbt_shards/cbt_shard_%d.pkl" % (shard_id), "rb") as fileptr:
            cbt_dataset = pickle.load(fileptr)
        break

        batch = []

        for idx in range(len(cbt_dataset) + len(wiki_dataset)):
            print(idx)
